...
def train(train_loader, test_loader, model, optimizer, epoch):
    ...
    for images, gts, masks, grays, edges in tqdm(train_loader):
        ...
        # bce = ratio * CE(coarse_prob, gts * masks) + edge_loss + ratio * CE(refine_prob, gts * masks)
        # 反向传播
        loss = bce
        loss.backward()
        # 防止梯度爆炸
    ...
    with torch.no_grad():
        ...
        for idx, (image, mask) in enumerate(test_loader):
            ...
            if not os.path.isdir(os.path.join(temp_dir, str(epoch))):
                os.makedirs(os.path.join(temp_dir, str(epoch)))
            save_dir = os.path.join(temp_dir, str(epoch), '%s.png' % filename)
            Image.fromarray(prediction).save(save_dir)
    test_loss = test_loss / len(test_loader.dataset)
    # 计算测试环节的BER值
    ber, acc = BER(os.path.join(temp_dir, str(epoch)), test_label_dir)
